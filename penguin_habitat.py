...
class PenguinHabitat(HabitatScene):
    ...
    def create_animals(self) -> list[Animal]:
        # Only one penguin if food or water games are active to avoid crowding
        ...
        return [
            Animal(x=100, y=200, layer_files=["penguin.png"], subfolder=_SUBFOLDER, scale=0.7,
                   default_facing_left=True, direction=1, speed=0.5, animate_fn=self._animate,
                   draw_fn=self._draw_animal, rect_size=(350, 350)),
            Animal(x=500, y=100, layer_files=["penguin.png"], subfolder=_SUBFOLDER, scale=0.7,
                   default_facing_left=True, direction=1, speed=0.5, animate_fn=self._animate,
                   draw_fn=self._draw_animal, rect_size=(350, 350))
        ]
    ...
    def _animate(animal: Animal) -> None:
        ...
        # Gentle waddle even when stationary
        animal.waddle_angle = math.sin(t * WADDLE_SPEED) * WADDLE_AMPLITUDE * (1.0 if speed_factor > 0 else 0.2)
        animal.y_offset = math.sin(t * WADDLE_SPEED * 2) * 2 * speed_factor

    def _draw_animal(self, animal: Animal, screen: pygame.Surface) -> None:
        if not animal.layers: return
        img = animal.layers[0]
        ...
        rotated_img, rect = self._movement.rotate_image(
            draw_img, getattr(animal, "waddle_angle", 0), (draw_pivot.x, draw_pivot.y)
        )
        rect.x += int(animal.x)
    ...
    def update(self, dt: float) -> None:
        ...
        w_rect, f_rect = self._get_station_rects()
        px = penguin.x + (penguin.layers[0].get_width() // 2 if penguin.layers else 0)
    ...
    def _draw_instructions(self, screen, rect, raw_txt):
