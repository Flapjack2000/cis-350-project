...
class TigerHabitat(HabitatScene):
    ...
    def __init__(self, manager: SceneManager) -> None:
        ...
        self._feed_active = "tiger_feed" in incomplete
    ...
    def update(self, dt: float) -> None:
        ...
        if self._water_active and w_rect.left <= tx <= w_rect.right:
            current_zone = "water"
        elif self._feed_active and f_rect.left <= tx <= f_rect.right:
            current_zone = "feed"
    ...
    def draw(self, screen: pygame.Surface) -> None:
        ...
        if self._water_active or self._feed_active:
            ...
            while words:
                line = ''
                while words and self._font.size(line + words[0])[0] < 180:
                    line += (words.pop(0) + ' ')
                lines.append(line)

            y_offset = w_rect.top + 20
            for line in lines:
                surf = self._font.render(line.strip(), True, (0, 0, 0))
        ...
        if self._water_active and self._water_level >= 100:
            ...
            self._complete_task("tiger_water")
